...
#define MAXIMUM_RUNNER_DISTANCE 900

@interface ACMultiplayerQuizServer ()
{
    int questionCount;
    float streakCount;
    BOOL shouldStopGame;
...
@implementation ACMultiplayerQuizServer
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    [self.readyPlayers addObject:self.hostPlayerID];
    
    if (self.readyPlayers.count == self.match.playerIDs.count+1)
    {
...
//    [[ACGraphics sharedGraphics] configureLayerForView:self.questionView];
    ...
    [self buildMultiplayerQuestionList];
}
...
{    
    ...
    [allPlayers addObject:self.hostPlayerID];
    ...
    for (NSString *thePlayerID in allPlayers)
    {
        ...
        if ([thePlayerID isEqualToString:self.hostPlayerID])
        {
            multiplayerRunner.alias = @"Me";
            multiplayerRunner.tagColor = [UIColor colorWithRed:1 green:0.84 blue:0 alpha:1.0];
        }
...
- (void)buildMultiplayerQuestionList
{
//    NSLog(@"SERVER IS BUILDING NEW QUESTION LIST");
//    
//    double start = CFAbsoluteTimeGetCurrent();
    
    MetaData *metaData = [[ACCDMgr sharedCDManager] metaDataObject];
    int totalQuestions = [metaData.totalQuestionCount intValue]-1;
    
    NSMutableArray *uids = [[NSMutableArray alloc] init];
    for (int i = 0; i < totalQuestions; i++)
    {
        [uids addObject:[NSNumber numberWithInt:i]];
    }
    
    int randomShuffleInt = (arc4random() % (3))+1;
    for (int i = 0; i < randomShuffleInt; i++)
    {
        [self shuffleArray:uids];
    }
    
    self.multiplayerQuestionList = uids;
    self.originalMultiplayerQuestionList = [[NSArray alloc] initWithArray:self.multiplayerQuestionList copyItems:YES];
    NSLog(@"FOUND: %i QUESTIONS:\n%@", self.originalMultiplayerQuestionList.count, [self.originalMultiplayerQuestionList description]);
    
//    NSLog(@"Number Of Questions Built: %i", [self.multiplayerQuestionList count]);
...
//    }
//    
//    NSMutableArray *questionIDNumbers = [[NSMutableArray alloc] init];
//    while (self.retrievableQuestionIDs.count > 0)
//    {
...
//        [questionIDNumbers addObject:rNum];
//    }
//    self.multiplayerQuestionList = questionIDNumbers;
//    NSLog(@"[SERVER] There are %i questions (for %i questions)", self.multiplayerQuestionList.count, TOTAL_NUMBER_OF_QUESTIONS);
//    double end = CFAbsoluteTimeGetCurrent();
//    double time = end - start;
//    NSLog(@"total time to process retrievable question IDs: %f", time);
    //NSLog(@"%@", [self.multiplayerQuestionList description]);
    
    [self sendQuestionListToClients];
...
//        //NSInteger n = (arc4random() % nElements) + 1;
//        
//        NSInteger n;
...
//    
//    NSLog(@"TOTAL SHUFFLE TIME: %g", result);
//}
...
- (void)answerSelected:(id)sender
{
    ...
    if ([answerKey isEqualToString:@"answer0"])
    {
        ...
        if (!winner)
        {
            ...
            NSLog(@"server sending update score (%i) to players", scoreToSend);
            [self sendUpdatedScoreToPlayers:scoreToSend];
            if (score >= MULTIPLAYER_MAX_SCORE)
            {
                winner = self.hostPlayerID;
                [self win];
            }
        }
        ...
        [self moveRunner:self.hostPlayerID distance:(newPoints - (newPoints * 0.1f)) time:2.0];
...
- (void)match:(GKMatch *)match didReceiveData:(NSData *)data fromPlayer:(NSString *)playerID
{
    ...
    switch (packet.dataType) {
        ...
        case kScore:
        {
            NSLog(@"Server saw score packet with score (%i)", packet.score);
...
- (void)sendQuestionListToClients
{
    NSData *questionData = [NSKeyedArchiver archivedDataWithRootObject:self.multiplayerQuestionList];
    ACDataPacket *dataPacket = [[ACDataPacket alloc] init];
    dataPacket.dataType = kQuestionList;
    dataPacket.payload = questionData;
    NSData *packet = [NSKeyedArchiver archivedDataWithRootObject:dataPacket];
    
    NSError *e;
    [self.match sendDataToAllPlayers:packet withDataMode:GKMatchSendDataReliable error:&e];
    if (e) NSLog(@"SEND QUESTIONS ERROR: %@", [e description]);
    
    NSLog(@"SEND QUESTION LIST (%iB)", [packet length]);
}
...
- (void)sendReadySignal
{
    ...
    if (e) NSLog(@"Error sending ready signal: %@", [e description]);
}
...
{
    if ([[segue identifier] isEqualToString:@"multiplayerServerEnd"])
    {
        ACMultiplayerEndGameController *end = (ACMultiplayerEndGameController *)[segue destinationViewController];
        if ([winner isEqualToString:self.hostPlayerID])
        {
            end.didWin = YES;
        }
        else
...
- (void)commenceResult
{
    [self performSegueWithIdentifier:@"multiplayerServerEnd" sender:self];
}
...
//    if (self.hasMultiplayerQuestionList && self.hasLoadedGame && self.allPlayersReady)
//    {
//        if (!self.readyPlayers) self.readyPlayers = [[NSMutableArray alloc] init];
//        [self sendReadySignal];
